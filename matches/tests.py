...
from matches.tasks import do_calculate_matches
from profiles.models import UserProfile

# These tests are used to test background tasks for calculating matches

class ClaimMatchTest(TestCase):
    def setUp(self):
        self.user1 = User.objects.create_user(username='user1', password='12345')
        Claim1.objects.create(user=self.user1,
        ...
        Claim2.objects.create(user=self.user1,
        ...
        Claim1.objects.create(user=self.user2,
        ...
        Claim2.objects.create(user=self.user2,
        ...
        Claim1.objects.create(user=self.user3,
        ...
        Claim2.objects.create(user=self.user3,
        ...
        Claim3.objects.create(user=self.user3,
        ...
        Claim1.objects.create(user=self.user4,
        ...
        Claim2.objects.create(user=self.user4,
    ...
    def test_calc_match_with_remaining_users(self):
        res = do_calculate_matches(self.user1.id)
        res = do_calculate_matches(self.user2.id)
        res = do_calculate_matches(self.user3.id)
        res = do_calculate_matches(self.user4.id)
        for m in Matches.objects.all():
        # for m in Matches.objects.filter(Q(user1=self.user2) | Q(user2=self.user2)):
